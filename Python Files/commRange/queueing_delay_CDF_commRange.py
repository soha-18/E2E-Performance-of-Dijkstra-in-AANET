...
for i, range in enumerate(commRange_values):
    group = queue_delay_vector[queue_delay_vector['commRange'] == range]
    group1 = filtered_queue_delay_vector_gpsr[filtered_queue_delay_vector_gpsr['commRange'] == range]
...
plt.legend()
plt.grid(True)
# plt.savefig('Queuing_Delay_NAC_A2G_scenario_CommRange_CDF_DSPR.pdf', format='pdf', dpi=1200)
# plt.savefig('Queuing_Delay_NAC_A2G_scenario_CommRange_CDF_GPSR.pdf', format='pdf', dpi=1200)
plt.show()
